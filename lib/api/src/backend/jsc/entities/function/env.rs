...
impl<T> FunctionEnv<T> {
        ...
        T: Any + 'static + Sized,
    {
        Self {
            handle: StoreHandle::new(
                store.as_store_mut().objects_mut().as_jsc_mut(),
    ...
    {
        self.handle
            .get(store.as_store_ref().objects().as_jsc())
            .as_ref()
            .downcast_ref::<T>()
            .unwrap()
    }
    ...
    {
        self.handle
            .get_mut(store.objects_mut().as_jsc_mut())
            .as_mut()
            .downcast_mut::<T>()
            .unwrap()
    }
...
impl<T> crate::FunctionEnv<T> {
    ...
    pub fn into_jsc(self) -> FunctionEnv<T> {
        match self.0 {
            crate::BackendFunctionEnv::Jsc(s) => s,
            _ => panic!("Not a `jsc` function env!"),
        }
    }

    /// Convert a reference to [`self`] into a reference to [`crate::backend::jsc::function::env::FunctionEnv`].
    pub fn as_jsc(&self) -> &FunctionEnv<T> {
        match self.0 {
            crate::BackendFunctionEnv::Jsc(ref s) => s,
            _ => panic!("Not a `jsc` function env!"),
        }
    }

    /// Convert a mutable reference to [`self`] into a mutable reference [`crate::backend::jsc::function::env::FunctionEnv`].
    pub fn as_jsc_mut(&mut self) -> &mut FunctionEnv<T> {
        match self.0 {
            crate::BackendFunctionEnv::Jsc(ref mut s) => s,
            _ => panic!("Not a `jsc` function env!"),
        }
    }
}
...
impl<'a, T> From<FunctionEnvMut<'a, T>> for crate::FunctionEnvMut<'a, T> {
    fn from(value: FunctionEnvMut<'a, T>) -> Self {
        Self(crate::BackendFunctionEnvMut::Jsc(value))
    }
}

impl<T> From<FunctionEnv<T>> for crate::FunctionEnv<T> {
    fn from(value: FunctionEnv<T>) -> Self {
        Self(crate::BackendFunctionEnv::Jsc(value))
    }
}
